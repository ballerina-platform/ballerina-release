...
pushd launcher > /dev/null 2>&1 
    kubectl create -f launcher-service.yaml -n ballerina-playground
    envsubst < launcher-deployment.yaml | kubectl create -n ballerina-playground -f -
popd > /dev/null 2>&1