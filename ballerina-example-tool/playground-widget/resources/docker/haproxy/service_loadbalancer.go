...
const (
	...
	lbPrivate                = "serviceloadbalancer/lb.private"
	lbRoutePath              = "serviceloadbalancer/lb.routePath"
...
var (
	...
	lbReloadInterval = flags.String("reload-interval", "10s", "Time interval to reload HAProxy configuration "+
		". Defaults to 1s")
)
...
type service struct {
	...
	// The path suffix to look for if path based routing should be done.
	RoutePath string
...
func (s serviceAnnotations) getPrivateService() (string, bool) {
	val, ok := s[lbPrivate]
	return val, ok
}

func (s serviceAnnotations) getRoutePath() (string, bool) {
	val, ok := s[lbRoutePath]
	return val, ok
}
...
func (cfg *loadBalancerConfig) write(services map[string][]service, dryRun bool) (err error) {
	...
	if err != nil {
		glog.Error("Couldn't compile HAProxy configuration. ERROR: ", err)
	}
...
func (cfg *loadBalancerConfig) reload() error {
	output, err := exec.Command("bash", "-c", cfg.ReloadCmd).CombinedOutput()
...
func (lbc *loadBalancerController) getServices() (httpSvc []service, httpsTermSvc []service, tcpSvc []service) {
	ep := []string{}
	services, err := lbc.svcLister.List()
	// If an err is returned by the kube API watcher, just return
	if err != nil {
		glog.Error("kube API is offline or error trying to make API call. not reloading config this round.")
		return
	}
	...
	for _, s := range services.Items {
		...
		// Ignore services with lb.private annotation from being added to HAProxy
		// load balancing
		if val, ok := serviceAnnotations(s.ObjectMeta.Annotations).getPrivateService(); ok {
			b, err := strconv.ParseBool(val)
			if err == nil && b == true {
				// glog.Infof("Ignoring service %v as it's set to Private.")
				continue
			}
		}
		...
		for _, servicePort := range s.Spec.Ports {
			...
			if val, ok := serviceAnnotations(s.ObjectMeta.Annotations).getRoutePath(); ok {
				// glog.Info("Adding routePath: ", val)
				newSvc.RoutePath = val
			}
...
func main() {
	...
	if *dry {
		...
		lbc.cfg.reload()
		reloadInterval, err := time.ParseDuration(*lbReloadInterval)
		if err != nil {
			glog.Fatalf("Error when reading reload interval: %s", *lbReloadInterval)
			reloadInterval = time.Second
		}

		glog.Infof("Reload interval is : %s", reloadInterval)
		wait.Until(lbc.worker, reloadInterval, wait.NeverStop)
	}
