...
@test "Create package '$PACKAGE_NAME:$VERSION' from SLP8." {
  run $SLP8/bin/ballerina new $PACKAGE_NAME
...
@test "Build package '$PACKAGE_NAME:$VERSION' from SLP8" {
  cd "$PACKAGE_NAME-$VERSION"
  run $SLP8/bin/ballerina build
  assert_line --partial "target/balo/$TEST_ORGANIZATION-$PACKAGE_NAME-any-$VERSION.balo"
  [ "$status" -eq 0 ]
  cd -
}
